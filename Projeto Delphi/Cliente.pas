...
begin
  ...
  begin
    EnviaMensagem('Utilize apenas Números ou * para pesquisar por Código do Cliente', 'Informação', mtInformation, [mbOK]);
    Key := #0;
  end
  ...
  begin
    EnviaMensagem('Utilize apenas Números ou * para pesquisar por RG', 'Informação', mtInformation, [mbOK]);
    Key := #0;
  end
  else if (ColunaPesquisa = '') and not CharInSet(Key,[#13,#42]) then
  begin
    EnviaMensagem('Utilize * ou escolha uma coluna da tabela.', 'Informação', mtInformation, [mbOK]);
    Key := #0;
  end
